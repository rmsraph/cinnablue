...
function push_results(results){
    var file = Gio.file_new_for_path('/usr/share/cinnamon/search_providers/duckduckgo@linuxmint.com/icon.png');
...
function on_result_selected(result){
    Util.spawn(['xdg-open', "https://duckduckgo.com/?t=lm&q=%s".replace("%s", result.id)]);
}
